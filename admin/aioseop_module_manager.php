	...
	class All_in_One_SEO_Pack_Module_Manager {
		...
		function __construct( $mod ) {
			...
			$reset_all = (
				...
				&& $_POST['Submit_All_Default'] != ''
			);
			$reset = (
				( isset( $_POST['Submit_Default'] )
				&& $_POST['Submit_Default'] != '' )
				|| $reset_all
			);
			$update = (
				isset( $_POST['action'] )
		 ...
		 * Module name is used for these automatic settings:
 		 ...
 		 * $this->modules[$module]
		 * @see aioseop_$module.php.
		 * @since 1.0.0
		 * @param String $mod, array $args.
		 * @return Boolean.
		 */
		function do_load_module( $mod, $args = null ) {
			$mod_path = apply_filters(
				"aioseop_include_$mod",
				AIOSEOP_PLUGIN_DIR . "modules/aioseop_$mod.php"
			);
			if ( ! empty( $mod_path ) )
				require_once( $mod_path );
			$ref = "aioseop_$mod";
			$classname = "All_in_One_SEO_Pack_" . strtr(
				ucwords(
					strtr(
						$mod,
						'_',
			...
			if ( is_user_logged_in()
				...
				&& current_user_can( 'aiosp_manage_seo' ) )
					add_action(
						'admin_bar_menu',
						array(
							$module_class,
			...
			if ( is_admin() ) {
				add_action(
					...
					Array( $module_class, 'add_menu' ),
					$module_class->menu_order()
				);
				add_action(
					'aiosoep_options_reset',
					Array( $module_class, 'reset_options' )
				);
				add_filter(
					'aioseop_export_settings',
					Array( $module_class, 'settings_export' )
				);
			}
			return true;
		}
		...
		function load_module( $mod ) {
			...
			$mod_enable = false;
			$fm_page = (
				$this->module_settings_update
				&& wp_verify_nonce( $_POST['nonce-aioseop'], 'aioseop-nonce' )
				&& isset($_REQUEST['page'])
				&& $_REQUEST['page'] == trailingslashit( AIOSEOP_PLUGIN_DIRNAME ) .
			...
			if ( $fm_page && ! $this->settings_reset ) {
				...
				{
					if ( $this->module_settings_update
						&& wp_verify_nonce( $_POST['nonce-aioseop'], 'aioseop-nonce' )
						&& $this->settings_reset_all )
							$feature_options = $this->modules['feature_manager']->default_options();
					else
