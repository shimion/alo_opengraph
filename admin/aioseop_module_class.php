...
//			$this->plugin_path['dir'] = plugin_dir_path( $this->file );
        ...
        function enqueue_metabox_scripts() {
            ...
            if (empty($screen))
                $bail = true;
            if (( $screen->base != 'post' ) && ( $screen->base != 'edit-tags' ) && ( $screen->base != 'toplevel_page_shopp-products' ))
                $bail = true;
                        ...
                        foreach ($v['display'] as $posttype) {
                            ...
                            if ($this->tabbed_metaboxes) {
                                ...
                                if (!empty($v['help_link']))
                                    $title .= "<a class='aioseop-help-text-link aioseop_meta_box_help' target='_blank' href='" . $lopts['help_link'] . "'><span>" . __('Help', 'all-in-one-seo-pack') . "</span></a>";
        ...
        function do_multi_input($args) {
            ...
            if (( $type == 'radio' ) || ( $type == 'checkbox' )) {
                $strings = Array(
                    'block' => "%s\n",
                    'group' => "\t<b>%s</b><br>\n%s\n",
                    'item' => "\t<label class='aioseop-option-setting-label'><input type='$type' %s name='%s' value='%s' %s> %s</label>\n",
        ...
        function get_option_html($args) {
            ...
            if (isset($opts['id']))
                $attr .= " id=\"{$opts['id']}\" ";
            // adding a different class to container span based on input 
            $label_additional_class = '';
            switch ($options['type']) {
                case 'multiselect':
                    $attr .= ' MULTIPLE';
                    $label_additional_class ='option-label-input-text';
                    $args['attr'] = $attr;
                    $args['name'] = $name = "{$name}[]";
                case 'select': 
        ...
        const DISPLAY_HELP_START = '<a class="aioseop-help-text-link" style="cursor:pointer;" title="%s" onclick="toggleVisibility(\'%s_tip\');"></a><label class="aioseop-label textinput">%s</label>';
        const DISPLAY_HELP_END = '<div class="aioseop-help-text-div" style="display:none" id="%s_tip"><label class="aioseop-help-text">%s</label></div>';
        const DISPLAY_LABEL_FORMAT = '<span class="aioseop-option-label %s" style="text-align:%s;vertical-align:top;">%s</span>';
        const DISPLAY_TOP_LABEL = "</div>\n<div class='aioseop-input aioseop_top_label'>\n";
        const DISPLAY_ROW_TEMPLATE = '<div class="aioseop-wrapper%s" id="%s_wrapper"><div class="aioseop-input">%s<span class="aioseop-option-input"><div class="aioseop-option-div" %s>%s</div>%s</span><p style="clear:left"></p></div></div>';

        /**
         * Format a row for an option on a settings page.
         */
        function get_option_row($name, $opts, $args) {
            $additional_class_input_label = '';
            switch($opts['type'])
            {
                case "textarea":
                    $additional_class_input_label = 'aioseop-option-label-additional';
                 break;
               case "select":
                    $additional_class_input_label = 'aioseop-option-label-additional';
                 break;
            ...
            if ($opts['label'] != 'none') {
                if (isset($opts['help_text'])) {
                    ...
                    $help_text = $opts['name'];
                $label_text = sprintf(All_in_One_SEO_Pack_Module::DISPLAY_LABEL_FORMAT, $additional_class_input_label,  $align, $help_text);
            } else
                $input_attr .= ' aioseop-no-label ';
            if ($opts['label'] == 'top')
        ...
        function display_options($location = null, $meta_args = null) {
            ...
            if (!empty($help_link))
                echo "<a class='aioseop-help-text-link aioseop_meta_box_help' target='_blank' href='" . $help_link . "'><span>" . __('Help', 'all-in-one-seo-pack') . "</span></a>";
            ...
            $container = "<div class='aioseop aioseop-options {$this->prefix}settings'>";
            ...
            foreach ($this->setting_options($location) as $k => $v) {
                if (isset($this->options[$k])) {
                    ...
                    switch ($type) {
                        ...
                        case 'radio':
                        case 'select':
                        default: if (!is_array($this->options[$k]))
        ...
        function display_settings_page($location = null) {
            ...
            <div class="wrap <?php echo get_class($this); ?>">
                <div id="aioseop-settings-header">
            ...
            ?>	<form id="aiosp-settings-form" name="dofollow" enctype="multipart/form-data" action="" method="post">
                    <div id="aioseop-top-button">
                        <div id="aiosp-ajax-settings-message"></div>
            <?php
            ...
            ?>
                    ...
                    <div class="aioseop-options-wrapper aioseop-settings-left">
            <?php
                ...
                foreach ($this->layout as $l => $lopts) {
                    if (!isset($lopts['tab']) || ( $this->current_tab == $lopts['tab'] )) {
                        ...
                        if (!empty($lopts['help_link']))
                            $title .= "<a class='aioseop-help-text-link aioseop_meta_box_help' target='_blank' href='" . $lopts['help_link'] . "'><span>" . __('Help', 'all-in-one-seo-pack') . "</span></a>";
                        add_meta_box($this->get_prefix($location) . $l . "_metabox", $title, array($this, 'display_options'), "{$this->prefix}settings", 'advanced', 'default', $lopts);
                    }
                }
            } else
