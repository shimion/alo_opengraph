...
if ( class_exists( 'WPSEO_Import_Hooks' ) ) {
	...
	class WPSEO_Import_AIOSEO_Hooks extends WPSEO_Import_Hooks {
		...
		public function show_import_settings_notice() {
			...
			$yoasturl = add_query_arg(
				array( '_wpnonce' => wp_create_nonce( 'wpseo-import' ) ),
				admin_url(
					'admin.php?page=wpseo_tools&tool=import-export&import=1&importaioseo=1#top#import-seo'
				)
			);
			$aiourl = add_query_arg(
				array( '_wpnonce' => wp_create_nonce( 'aiosp-import' ) ),
				admin_url( 'tools.php?page=aiosp_import' )
			);
			$aioseop_yst_detected_notice_dismissed = get_user_meta(
				get_current_user_id(),
		  	...
		  	if ( empty( $aioseop_yst_detected_notice_dismissed ) ) {
				echo
				'<div class="notice notice-warning row-title is-dismissible yst_notice">
					<p>',
						sprintf( esc_html__(
							'The plugin Yoast SEO has been detected.
								Do you want to %simport its settings%s
								into All in One SEO Pack?',
								'all-in-one-seo-pack'
						), sprintf( '<a href="%s">', esc_url( $aiourl ) ), '</a>' ),
					'</p>
				  </div>';
			}
			echo
				'<div class="error">
					<p>',
						sprintf(
							esc_html__(
								'The plugin All-In-One-SEO has been detected.
									Do you want to %simport its settings%s?',
								'wordpress-seo'
							),
						sprintf( '<a href="%s">', esc_url( $yoasturl ) ), '</a>' ),
					'</p>
				</div>';
		}

		/**
		 * Show deactivated notice.
		 * @since 1.0.0
		 */
...
function aiosp_seometa_settings_init() {
	...
	$_aiosp_seometa_admin_pagehook = add_submenu_page(
		'tools.php',
		__( 'Import SEO Data','all-in-one-seo-pack' ),
		__( 'SEO Data Import','all-in-one-seo-pack' ),
		'manage_options',
		'aiosp_import',
		'aiosp_seometa_admin'
	);
...
function aiosp_seometa_action() {
	...
	if ( ! empty( $_REQUEST['analyze'] ) ) {
		printf(
			...
			__( 'Analysis Results', 'all-in-one-seo-pack' )
		);
		$response = aiosp_seometa_post_meta_analyze(
			$_REQUEST['platform_old'],
			'All in One SEO Pack'
		);
		if ( is_wp_error( $response ) ) {
			printf(
				...
				__( 'Sorry, something went wrong. Please try again' )
			);
			return;
		}
		printf(
			__(
				'<p>Analyzing records in a %s to %s conversion&hellip;',
				'all-in-one-seo-pack'
			),
			esc_html( $_POST['platform_old'] ),
			'All in One SEO Pack'
		);
		printf(
			'<p>
				<b>%d</b> Compatible Records were identified
			</p>',
			$response->update
		);
		...
		printf(
			...
			__( 'Compatible data:', 'all-in-one-seo-pack' )
		);
		echo '<ol>';
	...
	printf(
		...
		__('Conversion Results', 'all-in-one-seo-pack')
	);
	$result = aiosp_seometa_post_meta_convert(
		stripslashes( $_REQUEST['platform_old'] ),
		'All in One SEO Pack'
	);
	...
	printf(
		'<p>
			<b>%d</b> Records were updated
		</p>',
		isset( $result->updated ) ? $result->updated : 0
	);
	printf(
		'<p>
			<b>%d</b> Records were ignored
		</p>',
		isset( $result->ignored ) ? $result->ignored : 0
	);
	return;
}

/**
 * Displays feedback to the user about compatible conversion.
 *
 * Displays feedback to the user about compatible conversion
 ...
 */
function aiosp_seometa_admin() { //The admin page output
	global $_aiosp_seometa_themes, $_aiosp_seometa_plugins, $_aiosp_seometa_platforms;
?>
	...
	<div class="wrap">
		...
		<h2>
			...
			<?php _e( 'Import SEO Settings', 'all-in-one-seo-pack' ); ?>
		...
		<p>
			<span class="description">
				...
				<?php
					printf(
						__(
							'Use the drop down below to choose which plugin or theme
								you wish to import SEO data from.',
							'all-in-one-seo-pack'
						)
					);
				?>
		...
		<p>
			<span class="description">
				...
				<?php
					printf(
						__(
							'Click "Analyze" for a list of SEO data
								that can be imported into All in One SEO Pack,
								along with the number of records that
								will be imported.',
							'all-in-one-seo-pack'
						)
					);
		...
		<p>
			<span class="description">
				...
				<?php
					printf(
						__(
							'Some plugins and themes do not share similar data,
								or they store data in a non-standard way.
								If we cannot import this data,
								it will remain unchanged in your database.
								Any compatible SEO data will be displayed for you to review.
								If a post or page already has SEO data in All in One SEO Pack,
								we will not import data from another plugin/theme.',
							'all-in-one-seo-pack'
						)
					);
				?>
		...
		<p>
			<span class="description">
				...
				<?php
					printf(
						__(
							'Click "Convert" to perform the import. After the import has completed,
								you will be alerted to how many records were imported,
								and how many records had to be ignored,
		...
		<p>
			<span class="row-title">
				...
				<?php
					printf(
						esc_html__(
							'Before performing an import, we strongly recommend that
								you make a backup of your site. We use and recommend
								%s BackupBuddy %s for backups.',
							'all-in-one-seo-pack'
						),
						sprintf(
		...
		<form action="<?php echo admin_url('tools.php?page=aiosp_import'); ?>" method="post">
			...
			<?php
				...
				printf(
					...
					__( 'Choose platform:', 'all-in-one-seo-pack' )
				);
				printf(
					'<optgroup label="%s">',
					__( 'Plugins', 'all-in-one-seo-pack' )
				);
				foreach ( $_aiosp_seometa_plugins as $platform => $data )
				{
					if( $platform != "All in One SEO Pack" )
						printf(
							...
							$platform,
							selected( $platform, $platform_old, 0 ),
							$platform
						);
				}
				printf( '</optgroup>' );
...
// FUNCTIONS
/**
 * Converts $old meta_key entries in the postmeta table into $new entries.
 *
 * checks to see what records for the $new meta_key already exist,
...
function aiosp_seometa_meta_key_convert( $old = '', $new = '', $delete_old = false ) {
	...
	// 	Sees which records we need to ignore, if any
	...
	if ( ! $exclude ) {
		$output->updated = $wpdb->update(
			$wpdb->postmeta,
			array( 'meta_key' => $new ),
			array( 'meta_key' => $old )
		);
		$output->deleted = $delete_old ? $wpdb->query(
			$wpdb->prepare( "DELETE FROM $wpdb->postmeta WHERE meta_key = %s", $old )
			) : 0;
		$output->ignored = 0;
	}
	...
	//	Else, do a more complex UPDATE and DELETE
		...
		$output->updated = $wpdb->query(
			$wpdb->prepare(
				"UPDATE $wpdb->postmeta SET meta_key = %s WHERE meta_key = %s AND post_id NOT IN ($not_in)",
				$new,
				$old
			)
		);
 ...
 */
/**
 * Cycles through all compatible SEO entries of two platforms.
 *
 * Cycles through all compatible SEO entries of two platforms,
 * performs a aiosp_seometa_meta_key_convert() conversion for each key, and returns
 * the results as an object.
...
function aiosp_seometa_post_meta_convert( $old_platform = '', $new_platform = 'All in One SEO Pack', $delete_old = false ) {
	...
	if ( empty( $_aiosp_seometa_platforms[$old_platform] )
		|| empty( $_aiosp_seometa_platforms[$new_platform] ) ) {
		$output->WP_Error = 1;
		return $output;
	}
	$output->updated = 0;
	...
	foreach ( ( array ) $_aiosp_seometa_platforms[$old_platform] as $label => $meta_key ) {
		...
		// skips iterations where no $new analog exists
		if ( empty( $_aiosp_seometa_platforms[$new_platform][$label] ) )
			continue;

		// sets $old and $new meta_key values
		...
		// updates total updated/ignored count
...
/**
 * Analyzes two platforms to see what Compatible elements they share,
...
function aiosp_seometa_import() {
	...
	$_aiosp_seometa_platforms = array_merge(
		$_aiosp_seometa_themes,
	 ...
	 * Inits hook.
	 *
	 * Fires after plugin functions are loaded.
	 * @since 0.9.10
	 */
...
/**
 * Initializes the SEO Meta activation hooks.
 * @since 1.0.0.
 */
...
/**
 * Deletes all SEO data, from every platform
 * @since 1.0.0.
 */
/*
